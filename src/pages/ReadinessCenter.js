      ...
      {open && (
        <ul className="checklist-items">
          {checklist.items.map((item, i) => (
            <ChecklistItem key={i} text={item} done={!!done[i]} onToggle={() => toggleItem(i)} />
          ))}
        </ul>
      )}
    </div>
...
function ReadinessCenter() {
  return (
    <div className="page">
      ...
      {PREMADE_CHECKLISTS.map((cl) => (
        <Checklist key={cl.id} checklist={cl} />
      ))}
    </div>
  );
}
